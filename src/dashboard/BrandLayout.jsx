...
// UPDATED: Full mobile responsive — sidebar overlay, header, main padding

import React, { useState } from "react";
import { NavLink, Outlet, useNavigate, Navigate } from "react-router-dom";
import {
  LayoutDashboard, TrendingUp, Package,
  FileText, User, LogOut, Menu, ShoppingBag, X,
} from "lucide-react";
...
export default function BrandLayout() {
  ...
  return (
    <div className="min-h-screen flex bg-gray-50" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      >
        {/* Brand header */}
        <div className={`px-5 py-5 bg-gradient-to-r ${theme.bg} to-transparent border-b border-white/10`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 min-w-0">
              <div
                className="w-10 h-10 rounded-xl flex items-center justify-center text-white font-black text-lg shadow-lg flex-shrink-0"
            ...
            <button
              ...
              className="lg:hidden w-7 h-7 rounded-lg bg-white/10 flex items-center justify-center text-white/60 hover:text-white ml-2 flex-shrink-0"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        {/* Top header */}
        <header
          className="bg-white border-b border-gray-100 px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between flex-shrink-0"
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-bold text-gray-900 text-sm sm:text-base">{brandName}</h2>
          </div>
          <div
