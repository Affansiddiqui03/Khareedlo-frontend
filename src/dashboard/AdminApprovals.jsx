            ...
            <>
              ...
              <div className="sm:hidden divide-y divide-gray-50">
                {filtered.map(p => {
                  ...
                  const imgSrc = p.image ? (p.image.startsWith("http") ? p.image : `https://khareedlo-backend-production.up.railway.app/${p.image}`) : null;
                  return (
                    <div key={p.product_id} className="p-4 space-y-3">
                      <div className="flex items-start gap-3">
                        <div className="w-16 h-16 rounded-xl bg-gray-100 overflow-hidden flex-shrink-0">
                          {imgSrc ? <img src={imgSrc} alt={p.product_name} className="w-full h-full object-cover" onError={e => e.target.style.display="none"} /> : <div className="w-full h-full flex items-center justify-center"><ImageIcon className="w-5 h-5 text-gray-300" /></div>}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-gray-800 text-sm line-clamp-2">{p.product_name}</p>
                          <p className="text-xs text-violet-600 font-medium mt-0.5">{p.brand_name}</p>
                          <p className="text-xs text-gray-400 mt-0.5">{p.gender} · PKR {Number(p.price).toLocaleString()}</p>
                        </div>
                        <span className={`flex-shrink-0 inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold border ${cfg.cls}`}>
                          <StatusIcon className="w-3 h-3" />{cfg.label}
                        </span>
                      </div>
                      {p.status === "PENDING" ? (
                        <div className="flex gap-2">
                          <button onClick={() => handleAction(p.product_id, "APPROVED")} disabled={isProc}
                            className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-emerald-600 text-white text-xs font-bold hover:bg-emerald-700 disabled:opacity-50">
                            <CheckCircle className="w-3.5 h-3.5" />{isProc ? "…" : "Approve"}
                          </button>
                          <button onClick={() => handleAction(p.product_id, "REJECTED")} disabled={isProc}
                            className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-red-50 text-red-600 border border-red-200 text-xs font-bold hover:bg-red-100 disabled:opacity-50">
                            <XCircle className="w-3.5 h-3.5" />{isProc ? "…" : "Reject"}
                          </button>
                          <button onClick={() => setPreview(p)} className="px-3 py-2.5 rounded-xl bg-gray-50 border border-gray-200 text-gray-500 hover:bg-gray-100">
                            <Eye className="w-4 h-4" />
                          </button>
                        </div>
                      ) : (
                        <button onClick={() => handleAction(p.product_id, "PENDING")} disabled={!!processing}
                          className="text-xs text-gray-400 hover:text-gray-600 underline">Reset to Pending</button>
                      )}
                    </div>
                  );
                })}
